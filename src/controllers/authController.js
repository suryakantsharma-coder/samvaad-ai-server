...
const register = async (req, res, next) => {
  try {
    const { email, password, name, role, hospitalId: bodyHospitalId, hospital: bodyHospital } = req.body;
    // Accept either hospitalId or hospital (string id or object with _id) for UI compatibility
    const hospitalId =
      bodyHospitalId != null
        ? bodyHospitalId
        : bodyHospital != null
          ? typeof bodyHospital === 'string'
            ? bodyHospital
            : bodyHospital._id || bodyHospital.id
          : undefined;
    if (!email || !password) {
