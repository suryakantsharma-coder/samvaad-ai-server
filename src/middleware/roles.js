...
/** Shorthand: hospital_admin only (admin cannot use this) */
const requireHospitalAdminOnly = (req, res, next) => {
  if (!req.user) {
    return res.status(401).json({ success: false, message: 'Authentication required' });
  }
  if (req.user.role !== ROLES.HOSPITAL_ADMIN) {
    return res.status(403).json({
      success: false,
      message: 'Only hospital admin can change user roles.',
    });
  }
  next();
};
