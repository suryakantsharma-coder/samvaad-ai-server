...
export const Settings = (): JSX.Element => {
  ...
  /** Auth tab: hospital users list vs create doctor login. */
  const [authSubTab, setAuthSubTab] = useState<"users" | "doctorAccount">(
    "users",
  );
  ...
  const [doctorAccountSuccess, setDoctorAccountSuccess] = useState(false);
  ...
  useEffect(() => {
    if (
      activeTab !== "auth" ||
      !isHospitalAdmin ||
      authSubTab !== "doctorAccount"
    ) {
      return;
    }
    const q = doctorSearchQuery.trim();
    if (!q) {
      setDoctorSearchResults(null);
      setDoctorSearchLoading(false);
      return;
    }
    const timer = window.setTimeout(() => {
      setDoctorSearchLoading(true);
      void searchDoctorsAsList(q, 1, 30)
        .then((list) => setDoctorSearchResults(list))
        .catch(() => setDoctorSearchResults([]))
        .finally(() => setDoctorSearchLoading(false));
    }, 400);
    return () => window.clearTimeout(timer);
  }, [activeTab, isHospitalAdmin, authSubTab, doctorSearchQuery]);
  ...
  const handleCreateDoctorAccount = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!doctorAccountForm.selectedDoctorId.trim()) {
      showWarning("Warning", "Search and select a doctor.");
      return;
    }
    if (doctorAccountForm.password !== doctorAccountForm.confirmPassword) {
      showWarning("Warning", "Passwords do not match.");
      return;
    }
    if (
      !doctorAccountForm.email.trim() ||
      !doctorAccountForm.name.trim() ||
      !doctorAccountForm.password
    ) {
      showWarning(
        "Warning",
        "Name, email, and password are required. Add an email to the doctor profile if it is missing.",
      );
      return;
    }
    const hospitalId = String(user?.hospital ?? "").trim();
    ...
    setDoctorAccountSuccess(false);
    try {
      await registerUserWithHospital({
        email: doctorAccountForm.email.trim(),
        password: doctorAccountForm.password,
        name: doctorAccountForm.name.trim(),
        role: "doctor",
        hospitalId,
      });
      await refreshUser();
      await handleGetHospitalUsers(hospitalId);
      setDoctorAccountSuccess(true);
      ...
      setDoctorSearchResults(null);
      showSuccess("Success!", "Doctor account created successfully.");
    } catch (err) {
      showError(
        "Error",
        err instanceof Error ? err.message : "Failed to create doctor account",
      );
    } finally {
            ...
            {activeTab === "auth" && isHospitalAdmin && (
              <div className="flex flex-col gap-6">
                ...
                >
                  <ToggleGroupItem
                    value="users"
                    className="px-5 py-[5px] rounded-[100px] font-title-4r data-[state=on]:bg-primary-2 data-[state=on]:text-white bg-transparent text-x-70"
                  >
                    Hospital users
                  </ToggleGroupItem>
                  <ToggleGroupItem
                ...
                {authSubTab === "doctorAccount" && (
                  <div className="bg-white rounded-[10px] p-6 flex flex-col gap-6">
                    ...
                    <p className="font-title-5l text-[#57575f] text-sm">
                      Search by doctor name or email. When you pick a result,
                      name and email are filled from the doctor profile—set a
                      password they will use to sign in.
                    </p>
                    {!String(user?.hospital ?? "").trim() ? (
                      <p className="font-title-4r text-[#57575f] py-4">
                        Your account has no linked hospital. Contact your admin
                        to get access.
                      </p>
                    ) : (
                      <form
                        onSubmit={handleCreateDoctorAccount}
                        className="flex flex-col gap-4 max-w-md"
                      >
                        {doctorAccountSuccess && (
                          <p className="font-title-4r text-primary-2 text-sm">
                            Account created. The doctor can sign in with this
                            email and password.
                          </p>
                        )}
                        <div className="flex flex-col gap-2">
                          ...
                          {!doctorAccountForm.selectedDoctorId.trim() ? (
                            <>
                              <div className="relative">
                                <Input
                                  ...
                                  className="h-[44px] w-full pr-10 px-4 py-2 bg-white border border-[#dedee1] rounded-[10px]"
                                  autoComplete="off"
                                />
                                <Search className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#57575f] pointer-events-none" />
                              </div>
                              {doctorSearchLoading && (
                                <p className="font-title-4r text-[#57575f] text-sm flex items-center gap-2">
                                  <Loader2 className="w-4 h-4 animate-spin" />
                                  Searching…
                                </p>
                              )}
                              {doctorSearchQuery.trim() &&
                                ...
                                doctorSearchResults.length === 0 && (
                                  <p className="font-title-4r text-[#57575f] text-sm">
                                    No doctors match. Try another name or email,
                                    or add the doctor on the Doctors page first.
                                  </p>
                                )}
                              {doctorSearchResults &&
                                doctorSearchResults.length > 0 && (
                                  <div className="max-h-[200px] overflow-y-auto rounded-[10px] border border-[#dedee1] bg-grey-light/30">
                                    {doctorSearchResults.map((d) => (
                                      <button
                                        key={d._id}
                                        type="button"
                                        className="w-full text-left px-4 py-3 font-title-4r text-black hover:bg-white border-b border-[#dedee1] last:border-0"
                                        onClick={() => {
                                          setDoctorAccountForm((prev) => ({
                                            ...prev,
                            ...
                            <div className="flex items-center justify-between gap-2 px-4 py-3 rounded-[10px] border border-[#dedee1] bg-grey-light/30">
                              <div className="min-w-0">
                                <p className="font-title-4m text-black truncate">
                                  {doctorAccountForm.name ||
                                    "Selected doctor"}
                                </p>
                                {doctorAccountForm.email ? (
                                  <p className="font-title-4r text-[#57575f] text-sm truncate">
                                    {doctorAccountForm.email}
                                  </p>
                                ) : (
                                  <p className="font-title-4r text-amber-700 text-sm">
                                    No email on profile—add one below or in
                                    Doctors before creating the account.
                                  </p>
                                )}
                              </div>
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                className="shrink-0 h-9 border-[#dedee1]"
                                onClick={() => {
                                  setDoctorAccountForm((prev) => ({
                                    ...prev,
                                    selectedDoctorId: "",
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <Input
                            type="password"
                            value={doctorAccountForm.password}
                            onChange={(e) =>
                              setDoctorAccountForm((prev) => ({
                                ...prev,
                                password: e.target.value,
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <Input
                            type="password"
                            value={doctorAccountForm.confirmPassword}
                            onChange={(e) =>
                              setDoctorAccountForm((prev) => ({
                                ...prev,
                                confirmPassword: e.target.value,
